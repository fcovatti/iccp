	...
	if(msg_rcv != NULL) {
		...
		if(signature==IHM_SINGLE_POINT_SIGN){
			...
			if(msg->tipo==30){
				...
				/*if(!(info->iq&0x01))
					state=(info->iq&0xF7)|0x01; //bit 3 is invalid timestamp
				else
					state=(info->iq&0xF6); //bit 0 is to set the value
				*/
				state=(info->iq&0xF7);
				...
				snprintf(query, QUERY_SIZE, "INSERT IGNORE INTO sde VALUES('%d','0','%02d-%02d-%02d','%02d:%02d:%02d', '%d', '%d', '0', NULL)", 
						msg->endereco, tm.tm_year+1900, tm.tm_mon, tm.tm_mday, tm.tm_hour, info->min, info->ms/1000, info->ms%1000, state);
#else
...
#else
	snprintf(flog,MAX_STR_NAME, "..\\logs\\hist_info-%04d%02d%02d%02d%02d.log", now.tm_year+1900, now.tm_mon+1, now.tm_mday,now.tm_hour, now.tm_min);
#endif
#else
	snprintf(flog,MAX_STR_NAME, "/tmp/hist_info-%04d%02d%02d%02d%02d.log", now.tm_year+1900, now.tm_mon+1, now.tm_mday,now.tm_hour, now.tm_min);
#endif
